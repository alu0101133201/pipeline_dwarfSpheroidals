...
##Eventhough we will have a single definition, we need to build the ring based on the positions of each of the windows
#This will depend on the filter. Let's make it on a similar way as the multi detector pipeline
...
if [ -f $ringtempDone ]; then
  ...
  if [ "$USE_COMMON_RING" = true ]; then
    ...
    prepareRingTemplate $commonRingDefinitionFile $INDIRo/night1 $DIR $filter
  else
    prepareRingTemplate $firstRingDefinitionFile $INDIRo/night1 $DIR $filter
    prepareRingTemplate $secondRingDefinitionFile $INDIRo/night1 $DIR $filter
  fi
...
oneNightPreProcessing() {
  ...
  local framesForCommonReductionDone=$framesForCommonReductionDir/done_"$filter"_n"$currentNight".txt
  ...
  if [ -f $renamedone ]; then
    ...
    for i in $currentINDIRo/*.fits; do
      ...
      for h in $(seq 0 $num_ccd); do
        if [ $h -eq 0 ]; then
          eval "astfits $nameWithEscapedSpaces --copy=$h --primaryimghdu -o $out"
  ...
  if [ -f $skydone ]; then
    ...
    for i in $(ls -v $currentINDIR/*.fits ); do
      ...
      air=$(astfits $i -h0 --keyvalue=$airMassKeyWord 2>/dev/null | awk '{print $2}')
	    if [[ $air == "n/a" ]]; then
    	  ...
    	  astfits $i -h0 --write=$airMassKeyWord,$air,"Updated from secz"
      fi
  ...
  ringdir=$BDIR/ring_n"$currentNight"
  if ! [ -d $ringdir ]; then mkdir $ringdir; fi
  # We create the .fits ring image based on how the normalisation is going to be done
  for h in $(seq 1 $num_ccd); do
    if [[ "$USE_COMMON_RING" = true && ! -f "$ringdir/ring_ccd"$h".fits"  ]]; then
      base="${commonRingDefinitionFile%.txt}"
      cp $DIR/"$base"_ccd"$h".txt $ringdir/ring_ccd"$h".txt 
      astmkprof --background=$mbiascorrdir/"$objectName"-Decals-"$filter"_n"$currentNight"_f1.fits --backhdu=$h --mforflatpix --mode=img --type=uint8 --circumwidth=$ringWidth --clearcanvas -o $ringdir/ring_ccd"$h".fits $ringdir/ring_ccd"$h".txt
      astfits $ringdir/ring_ccd"$h".fits --copy=1 -o $ringdir/ring.fits
    else
  ...
  if $RUNNING_FLAT; then
    ...
    if [ -f $noiseit2done ]; then
      ...
      printf "%s\n" "${frameNames[@]}" | parallel -j "$num_cpus" runNoiseChiselOnFrame {} $flatit1imadir $noiseit2dir $blockScale "'$noisechisel_param'"
  ...
  if [ -f $noiseit2WholeNightdone ]; then
    ...
    printf "%s\n" "${frameNames[@]}" | parallel -j "$num_cpus" runNoiseChiselOnFrame {} $flatit1WholeNightimaDir $noiseit2WholeNightDir $blockScale "'$noisechisel_param'"
  ...
  if $RUNNING_FLAT; then
    ...
    if [ -f $noiseit3done ]; then
      ...
      printf "%s\n" "${frameNames[@]}" | parallel -j "$num_cpus" runNoiseChiselOnFrame {} $flatit2imadir $noiseit3dir $blockScale "'$noisechisel_param'"
  ...
  if [ -f $noiseit3WholeNightdone ]; then
    ...
    printf "%s\n" "${frameNames[@]}" | parallel -j "$num_cpus" runNoiseChiselOnFrame {} $flatit2WholeNightimaDir $noiseit3WholeNightDir $blockScale "'$noisechisel_param'"
...
if [ -f $gaincordone ]; then
  ...
  printf "%s\n" "${frameNames[@]}" | parallel -j "$num_cpus" gainCorrection {} $ringDir $gaincordir $blockScale "'$noisechisel_param'" 
  ...
  if [ "$astrometrizedFolder" = "NONE" ]; then
    ...
    if [ -f $scampdone ]; then
      ...
      for ((i = 1; i <= numOfSextractorPlusScampIterations; i++)); do
        ...
        if [ -f $CDIR/scamp_"$i".cfg ]; then
          scampcfg=$CDIR/scamp_"$i".cfg
        fi
...
noiseskydone=$noiseskydir/done_"$filter"_ccd"$h".txt
imagesAreMasked=true
computeSky $smallPointings_maskedDir $noiseskydir $noiseskydone $MODEL_SKY_AS_CONSTANT wholeImage $polynomialDegree $imagesAreMasked $BDIR/ring $USE_COMMON_RING $keyWordToDecideRing $keyWordThreshold $keyWordValueForFirstRing $keyWordValueForSecondRing $ringWidth YES $blockScale "'$noisechisel_param'" "'$maskParams'"
...
if [ -f $coaddDone ]; then
  ...
  imagesAreMasked=true
	computeSky $subSkyPointings_maskedDir $noisesky_prephot $noisesky_prephotdone $MODEL_SKY_AS_CONSTANT wholeImage $polynomialDegree $imagesAreMasked $ringDir $USE_COMMON_RING $keyWordToDecideRing $keyWordThreshold $keyWordValueForFirstRing $keyWordValueForSecondRing $ringWidth YES $blockScale "'$noisechisel_param'" "'$maskParams'"
  #exit 0
...
values=("$numberOfFramesCombined" "$numberOfNights" "$initialTime" "$meanTime" "$finalTime" "$filter" "$lowerVignettingThreshold" "$upperVignettingThreshold" "$saturationThreshold" "$surveyForPhotometry" "$RUNNING_FLAT" "$halfWindowSize")
comments=("" "" "" "" "" "" "" "" "" "" "" "Running flat built with +-N frames")
...
if [ -f $photCorrfullGridDone ]; then
  echo "\nFrames already multiplied by common calibration factor"
else
  for frame in $subSkyNoOutliersPxDir/*.fits; do
    base=$( basename $frame )
    astarithmetic $frame -h1 $alpha x float32 -o$photCorrfullGridDir/$base
  done
  echo done > $photCorrfullGridDone
fi
