...
source "$pipelinePath/pipeline_TST_functions_vic.sh"
...
# Rebinned data
tileSize=30
# astmkprof --kernel=gaussian,3.5,4 --oversample=1 -o$ROOTDIR/"$objectName"/kernel.fits 
...
if [ -f $catdone ]; then
  ...
  python3 $pythonScriptsPath/createDS9RegionsFromGaiaCatalogue.py $catName $catRegionName 
...
if [ -f $indexdone ]; then
  echo -e "\n\tGaia DR3 indexes are already created\n"
else
