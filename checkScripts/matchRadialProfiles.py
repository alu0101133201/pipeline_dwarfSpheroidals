...
import sys
import pandas as pd
##Filter differences for a mean color g-r=0.5
filter_factors=pd.DataFrame({
    'filter':['u','g','r','i','z'],'SDSS':[0.22,0.032,0.0049,0.0016,0.0076],
    'DECaLS':[0.0,-0.02,-0.041,-0.0041,-0.031],'PANSTARRS':[0.0,-0.045,-0.00042,0.0043,0.011]})
...
def main():
    ...
    parser.add_argument('filter', help='Filter used')
    parser.add_argument('survey', help='Survey name (e.g., DECaLS, PANSTARRS, SDSS)')
    parser.add_argument('output_file', help='Output file to write scaling factor')
    ...
    parser.add_argument('--plot_name', help='Name of the comparison plot',default='profile_comparison.pdf')
    ...
    try:
        ...
        if args.save_plot:
            ...
            plot_comparison(radius1_arcsec, data1['mean'], sb1_full,
                          ...
                          scale_factor, args.r_min, args.r_max, args.plot_name)

        ###Multiply by difference filter factor and write it into output_file.txt
        filter_factor=filter_factors.loc[filter_factors['filter']==args.filter,args.survey].values[0]
        final_scale_factor=scale_factor*10**(0.4*filter_factor)
        
        with open(args.output_file,'w') as f:
            f.write(f"{final_scale_factor}\n")
        print(f"\nFinal scaling factor after filter correction ({args.survey} {args.filter}): {final_scale_factor:.6f}")
    except Exception as e:
