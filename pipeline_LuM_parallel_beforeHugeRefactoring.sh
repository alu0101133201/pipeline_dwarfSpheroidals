...
warpImage() {
  ...
  # We do not need to store the coordinates or any additional data. If we warp the cropped frame the same way back, we will obtain
  # exactly the same frame. I have checked it.
  ...
  tmpFile2=$entiredir"/$currentIndex"_temp2.fits
  ...
  astarithmetic $entiredir/"$currentIndex"_swarp1.fits -h0 $tmpFile1 -h1 0 eq nan where -o$tmpFile2
  ...
  astcrop $tmpFile2 --center=$centralRa,$centralDec --mode=wcs --width=$detectorHeightDeg/3600,$detectorWidthDeg/3600  -o $entiredir/entirecamera_"$currentIndex".fits
...
# Functions for Photometric calibration
computeSkyForFrame(){
  base=$1
  entiredir=$2
  noiseskydir=$3
...
if [ -f $entiredone ]; then
  ...
  printf "%s\n" "${imagesToWarp[@]}" | parallel -j "$num_cpus" warpImage {} $swarpcfg $ra $dec $coaddSizePx $entiredir
...
for h in 0; do
    ...
    if [ -f $noiseskydone ]; then
      ...
      printf "%s\n" "${framesToComputeSky[@]}" | parallel -j "$num_cpus" computeSkyForFrame {} $entiredir $noiseskydir
    ...
    for a in $(seq 1 $totalNumberOfFrames); do
        base="entirecamera_"$a.fits
        i=$noiseskydir/"entirecamera_"$a.txt
        me=$(awk 'NR=='1'{print $1}' $i)
        input=$entiredir/$base
...
referenceImagesForMosaic=$entiredir
...
if [ -f $buildMosaicDone ]; then
  ...
  # I apply this because the resolution is not relevant and I don't want to spend a lot of time here
  scaleFactor=0.25 
  decalsPxScale=1.048
  mosaicSize=9500
...
framesForCalibrationDir=$entiredir
...
selectStarsAndRangeForCalibrateOurData(){
  a=$1
  framesForCalibrationDir=$2
  mycatdir=$3
  sigmaForStdSigclip=$4
  iterationsForStdSigClip=$5
  sigmaForPLRegion=$6
...
if [ -f $mycatdone ]; then
  ...
  printf "%s\n" "${framesToUse[@]}" | parallel -j "$num_cpus" selectStarsAndRangeForCalibrateOurData {} $framesForCalibrationDir $mycatdir $sigmaForStdSigclip $iterationsForStdSigClip $sigmaForPLRegion
  echo done > $mycatdone
fi
...
if [ -f $exposuremapdone ]; then
  ...
  echo "This has to be reimplemented"
  
  # Pure exposure map
  # framesDir=$BDIR/pointings
  # for a in $(seq 1 $totalNumberOfFrames); do
  #   i=$framesDir/$a.fits
