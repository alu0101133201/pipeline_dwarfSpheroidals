    ...
    with open(currentFile, 'r') as f:
        ...
        for line in f:
            ...
            if (numberOfFields == 3):
                ...
                raise Exception("Wrong number of fields in the file of background estimation. Expected 3 (constant estimation) or 2 (plane/higher-order estimation)")
    return(dataDict)
...
def obtainPotentialBadFiles(dataDict, medianValues, stdValues):
    ...
    for currentKey in dataDict:
        ...
        for currentImageValue in currentValues:
            if (np.abs(currentImageValue[1] - median) >= (3*std)):
...
def createAndWriteHistograms(dataDict, outputFolder):
    for currentKey in dataDict.keys():
        ...
        ax.hist(dataToShow)
        plt.savefig(outputFolder + "/" + currentKey + ".pdf")
# --- #
