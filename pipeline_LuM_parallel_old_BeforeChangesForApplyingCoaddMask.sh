...
for h in 0; do
    ...
    subskydone=$subskydir/done_"$filter"_ccd"$h".txt
    ...
    for a in $(seq 1 $totalNumberOfFrames); do
        ...
        input=$astroimadir/$base
        output=$subskydir/$base
...
# Warp with these headers
echo -e "\n ${GREEN} ---Warping and correcting distorsion--- ${NOCOLOUR}"

# Now we need to warp the data so we can place it in a proper grid and we can improve the astrometry thanks to scamp

# In the two following blocks of code we warp the $subskydir images two times
# The first one (subsky_astroCorr) regrids to an equivalent dimensions but warps it to improve the astrometry
# The second one (pointings) regrids into a bigger grid where the final coadd is going to be placed

# The objective of this is to not use de pointings (huge image) in the photometric calibration, but the subsky_astroCorr (smaller image)
# and then apply the calibration to the pointings and coadd them.


entiredone=$BDIR/pointings/done_.txt
entiredir=$BDIR/pointings
swarpcfg=$ROOTDIR/"$objectName"/config/swarp.cfg
if ! [ -d $entiredir ]; then mkdir $entiredir; fi
cp $sexdir/*.head $subskydir
if [ -f $entiredone ]; then
    ...
    for a in $(seq 1 $totalNumberOfFrames); do
        base="$a".fits
        #copy image
        #cp $subskydir/$base $entiredir/tmp_$base

        # Resample into one single image
        SWarp -c $swarpcfg $subskydir/$base -CENTER $ra,$dec -IMAGE_SIZE $coaddSizePx,$coaddSizePx -IMAGEOUT_NAME $entiredir/swarp1.fits -WEIGHTOUT_NAME $entiredir/swarp_w1.fits -SUBTRACT_BACK N -PIXEL_SCALE 1.164 -PIXELSCALE_TYPE  MANUAL
        # Mask bad pixels
        astarithmetic $entiredir/swarp_w1.fits -h0 set-i i i 0 lt nan where -otemp1.fits
        astarithmetic $entiredir/swarp1.fits -h0 temp1.fits -h1 0 eq nan where -o$entiredir/entirecamera_"$a".fits
        
        # rm $subskydir/$base
        # -----------------------------------------------------------------
    done
    rm $entiredir/swarp_w1.fits $entiredir/swarp1.fits
    echo done > $entiredone
fi

subsky_astroCorrDone=$BDIR/subsky_astroCorr/done_.txt
subsky_astroCorr=$BDIR/subsky_astroCorr
swarpcfg=$ROOTDIR/"$objectName"/config/swarp.cfg
if ! [ -d $subsky_astroCorr ]; then mkdir $subsky_astroCorr; fi
if [ -f $subsky_astroCorrDone ]; then
    echo -e "\nsubs_sky_it1 images already with astromety corrected using scamp-swarp (stored in subsky_astroCorr)\n"
else
    for a in $(seq 1 $totalNumberOfFrames); do
        base="$a".fits

        SWarp -c $swarpcfg $subskydir/$base -CENTER_TYPE ALL  -IMAGE_SIZE $detectorHeight,$detectorWidth -IMAGEOUT_NAME $subsky_astroCorr/swarp1.fits -WEIGHTOUT_NAME $subsky_astroCorr/swarp_w1.fits -SUBTRACT_BACK N -PIXEL_SCALE 1.164 -PIXELSCALE_TYPE  MANUAL
        # Mask bad pixels
        astarithmetic $subsky_astroCorr/swarp_w1.fits -h0 set-i i i 0 lt nan where -otemp1.fits
        astarithmetic $subsky_astroCorr/swarp1.fits -h0 temp1.fits -h1 0 eq nan where -o$subsky_astroCorr/$base
    done
    echo done > $subsky_astroCorrDone
    rm $subsky_astroCorr/swarp_w1.fits $subsky_astroCorr/swarp1.fits
fi
...
# Parameter for deciding the selection region (half-max-rad region)
sigmaForPLRegion=1
...
totalNumberOfFrames=$( ls $BDIR/pointings/*.fits | wc -l )

echo -e "\n ${GREEN} ---Preparing Decals data--- ${NOCOLOUR}"
referenceImagesForMosaic=$subsky_astroCorr
...
if [ -f $donwloadMosaicDone ]; then
  ...
  for a in $(seq 1 $totalNumberOfFrames); do 
    if [ $a -eq 1 ]; then
      # Remove the brick map. This is done to avoid problems with files of previous executions
      rm $frameBrickCorrespondenceFile
    fi
    ...
    # Creating the map between frames and bricks to recover it in the photometric calibration
    echo $base $bricksOfTheFrame >> $frameBrickCorrespondenceFile 
  done 
...
if [ -f $addBricksDone ]; then
  ...
  for file in "$decalsImagesDir"/*_g.fits; do
    # The following lines are dependant on the name of the decals images, which is defined in the python script "./decals_GetAndDownloadBricks.py"
...
# Here is build without modifing the resolution of the decals frames, but it could be downgraded in order to save some time and space
# Is built with the (g+r)/2 so later we can compare it with our data, but for the pipeline would work using 'g' or 'r' as well, because strictly we only need the field
...
if [ -f $buildMosaicDone ]; then
  ...
  # I apply this because the resolution is not relevant and I don't want to spend hours here
  scaleFactor=0.25 
  decalsPxScale=1.048
  mosaicSize=9500
  ...
  rm $swarpedImagesDir/swarp_w1.fits
  rm $swarpedImagesDir/swarp1.fits
  rm ./temp1.fits
  sigma=2
...
#### OBTAINING GAIA CATALOGUE FOR OUR FIELD  ####
echo -e "\n ${GREEN} ---Downloading GAIA catalogue--- ${NOCOLOUR}"

# retcatdir=retrived-catalog
retcatdone=$BDIR/downloadedGaia_done_"$n".txt

mos=$DIR/mosaic/mosaic.fits
ref=$mos
...
### DECALS POINT-LIKE SOURCES FOR CALIBRATION ###


echo -e "\n ${GREEN} --- Selecting stars and range for Decals--- ${NOCOLOUR}"
...
getParametersFromHalfMaxRadius() {
  ...
  astnoisechisel $image -h1 -o $tmpFolder/det.fits --convolved=$tmpFolder/convolved.fits --tilesize=30,30 --detgrowquant=0.95 --erode=4 1>/dev/null
...
chooseBrickToUse() {
  ...
  if [ -n "$bestBrick" ]; then
    ...
    for i in $bricks; do
      ...
      # ****** Decision note *******
      # The criteria for selecting the best brick is having at least 90 stars and, from those that fulfill this condition, the lower std
...
# These frames are the ones used for computing the "alphas" (the calibration factors)
# Take into account that later the alpha is applied to the "POINTINGS", because these are the ones warped properly
# But, since both (pointings and subsky_astroCorr) are the same (the only difference is that they are warped into different grids)
# I will use the subsky_astroCorr so I can work with much smaller frames and everything is faster
framesForCalibrationDir=$subsky_astroCorr
...
### MY POINT-LIKE SOURCES FOR CALIBRATION ###
echo -e "\n ${GREEN} ---Selecting stars and range for our data--- ${NOCOLOUR}"

mycatdir=$BDIR/my-catalog-halfmaxradius
mycatdone=$mycatdir/done_ccd"$h".txt
if ! [ -d $mycatdir ]; then mkdir $mycatdir; fi
if [ -f $mycatdone ]; then
    echo -e "\nSources for photoemtric calibration are already extracted for my image\n"
else
  ...
  for a in $(seq 1 $totalNumberOfFrames); do
    ...
    astnoisechisel $i -h1 -o det.fits
    astsegment det.fits -o seg.fits --snquant=0.1 --gthresh=-10 --objbordersn=0  --minriverlength=3
    # all sources
    astmkcatalog seg.fits --ra --dec --magnitude --half-max-radius --sum --clumpscat -o $mycatdir/"$base".txt
    # now match with Gaia to select range automatically
    ...
    # now I get the mean and the std of the distribution of the HSR
    ...
    rm det.fits seg.fits
  done 
  echo done > $mycatdone
fi


### Match between Decals and my point-like sources ###
echo -e "\n ${GREEN} ---Matching our data and Decals--- ${NOCOLOUR}"

matchdir2=$BDIR/match-decals-my
...
### DECALS: Build the catalogue ###
echo -e "\n ${GREEN} ---Building Decals catalogue for calibration stars--- ${NOCOLOUR}"

decalsdir=$BDIR/decals-aperture-catalog
...
if [ -f $decalsdone ]; then
  ...
  for a in $(seq 1 $totalNumberOfFrames); do
    ...
    # It is already saved as mean value of the PL sources
...
### My Data: Build the catalogue ###
echo -e "\n ${GREEN} ---Building our catalogue for calibration stars--- ${NOCOLOUR}"

ourDatadir=$BDIR/ourData-catalogs-apertures
...
### Match between decals catalogue and our data catalogues ###
...
if [ -f $matchdir2done ]; then
  ...
  for a in $(seq 1 $totalNumberOfFrames); do
    ...
    astmatch $decalsdir/decals_"$base".cat --hdu=1 $i --hdu=1 --ccol1=RA,DEC --ccol2=RA,DEC --aperture=1/3600 --outcols=aRA,aDEC,aMAGNITUDE,aSUM,bMAGNITUDE,bSUM -o$out

    # I'm adding the ra an dec of my data as well in order to do the deltamag deltadec plot
    # astmatch $decalsdir/decals_"$base".cat --hdu=1 $i --hdu=1 --ccol1=RA,DEC --ccol2=RA,DEC --aperture=1/3600 --outcols=aRA,aDEC,bRA,bDEC,aMAGNITUDE,aSUM,bMAGNITUDE,bSUM -o$out
  done
  echo done > $matchdir2done
fi
...
# As stated before, Now that we have the calibration factors we come back to the pointings, which are
# warped properly. (I was using the same images but warped into a smaller grid) 
pointingsDir=$BDIR/pointings

muldir=$BDIR/phot-corr-dir-fullGrid
muldone=$muldir/done_ccd"$h".txt
...
if [ -f $muldone ]; then
    ...
    for a in $(seq 1 $totalNumberOfFrames); do
        base="$a".fits
        f=$pointingsDir/"entirecamera_$a.fits"
        
        alpha_cat=$alphatruedir/alpha_"$objectName"_Decals-"$filter"_"$a"_ccd"$h".txt
        alpha=$(awk 'NR=='1'{print $1}' $alpha_cat)
        echo astarithmetic $f -h1 $alpha x float32 -o $muldir/$base
        astarithmetic $f -h1 $alpha x float32 -o $muldir/$base
        # rm $f
    done
    echo done > $muldone
fi

# I will also calibrate the other set of images (the ones used for the calculus of the calibration factors)
# in order to estimate the std in a swifter way.
subsky_astroCorrDir=$BDIR/subsky_astroCorr
muldir=$BDIR/phot-corr-dir-frameGrid
muldone=$muldir/done_ccd"$h".txt
if ! [ -d $muldir ]; then mkdir $muldir; fi
if [ -f $muldone ]; then
    echo -e "\nMultiplication for alpha in the frames (small grids) done for extension $h\n"
else
    for a in $(seq 1 $totalNumberOfFrames); do
        base="$a".fits
        f=$subsky_astroCorrDir/$base
...
echo -e "\n ${GREEN} ---Computing STD for the calibrated frames--- ${NOCOLOUR}"

# Compute rms and of the photometrized frames
muldir=$BDIR/phot-corr-dir-frameGrid # We use the small grid for doing it faster

noiseskydir=$BDIR/noise-sky-after-photometry
noiseskyFile=$noiseskydir/noiseSky.txt
...
if [ -f $noiseskydone ]; then
  ...
  for a in $(seq 1 $totalNumberOfFrames); do
    ...
    sky=$(echo $base | sed 's/.fits/_sky.fits/')
    out=$(echo $base | sed 's/.fits/.txt/')
    astnoisechisel $i --checksky -o $noiseskydir/$base $noisechisel_param       #--tilesize=20,20 --interpnumngb=5 --dthresh=0.1 --snminarea=2
    # save both mean and std	
    m=$(aststatistics $noiseskydir/$sky -hSKY --sigclip-mean)
    s=$(aststatistics $noiseskydir/$sky -hSTD --sigclip-mean)
    echo "$base $m $s" >> $noiseskyFile
    rm -f $noiseskydir/$sky
  done	
echo done > $noiseskydone									
fi
...
python3 find_rms_min.py "$filter" 1 $totalNumberOfFrames $h $DIR $objectName

### Calculate the weights for the images based on the minimum rms ###
echo -e "\n ${GREEN} ---Computing weights for the frames--- ${NOCOLOUR}"

# Once we have the rms computed for the frames we switch to the full grid
muldir=$BDIR/phot-corr-dir-fullGrid
...
if [ -f $wdone ]; then
  ...
  for a in $(seq 1 $totalNumberOfFrames); do
    ...
    rms_f=$( grep "^$base " $noiseskyFile | awk '{print $3}' )
    weight=$(astarithmetic $rms_min $rms_f / --quiet)
    echo "$weight" > $wdir/"$objectName"_Sloan-"$filter"_"$a"_ccd"$h".txt    #  saving into file
...
# Sometimes when I load all the frames it explodes. I think it's because some error in the memory free process. 
# I remove the previous memory maps manually
rm -rf "./gnuastro_mmap" 
...
if [ -f $mowdone ]; then
    ...
    for a in $(seq 1 $totalNumberOfFrames); do
        base="$a".fits
        tmp_ab=$mowdir/"$objectName"_Sloan-"$filter"_"$a"_ccd"$h"_maskabove.fits
        wom=$mowdir/$base
        echo $base
        echo $tmp_ab
        echo $wom
        ...
        # save the new mask
        mask=$mowdir/"$objectName"_Sloan-"$filter"_"$a"_ccd"$h"_mask.fits
...
# Doing the final coadd performing the weighted mean
coaddir=$baseCoaddir/stdWeighted1
coaddName=$coaddir/"$objectName"_Decals-"$filter".fits
...
maskName="$coaddir/$objectName-weighted-mask.fits"
if [ -f $maskName ]; then
...
# --- Coadd and its mask ---
coaddDir=$baseCoaddir/pureMedian
coaddName="$coaddDir/$objectName-sigclip.fits"

if ! [ -d $coaddDir ]; then mkdir $coaddDir; fi
if [ -f $coaddName ]; then
  echo -e "\nThe coadd using the median is already done\n"
else
  # ****** Decision note *******
  # The sigma for the rejections in the coadd has been chosen to be two. 
  # Caveat: this decision has been taken while working with only 1 night (36 frames)
  # those are not much frames and with the dithering patter we need a more aggresive sigma-clip in
  # order to remove the outliers
  sigma=2
  astarithmetic $(ls -v $muldir/*.fits )  $(ls -v $muldir/*.fits | wc -l) -g1 $sigma 0.2 sigclip-median -o $coaddName
fi
maskName="$coaddDir/$objectName-sigclip-mask.fits"
if [ -f $maskName ]; then
  echo "The mask of the median coadd is already done"
else
  astnoisechisel $coaddName $noisechisel_param -o $maskName
fi
...
exposuremapdone=$exposuremapDir/done_"$k".txt
...
if [ -f $exposuremapdone ]; then
  ...
  # Pure exposure map
  framesDir=$BDIR/subsky_astroCorr
  for a in $(seq 1 $totalNumberOfFrames); do
    i=$framesDir/$a.fits
  ...
  astarithmetic $(ls -v $exposuremapDir/*.fits) $(ls $exposuremapDir/*.fits | wc -l) number -g1 -o$exposuremapDir/exposureMap_NoNans.fits

  # Compination taking into account all the masked pixels
  # astarithmetic $(ls -v $mowdir/*.fits) $(ls $mowdir/*.fits | wc -l) number -g1 -o$coaddir/"exposureMap_initial.fits"
  # astarithmetic $coaddir/"exposureMap_initial.fits" -h1 float32 -o$coaddir/"exposureMap_tmp.fits"
  # astarithmetic $coaddir/"exposureMap_tmp.fits" -h1 $totalNumberOfFrames / -o $coaddir/"exposureMap_WithNans.fits"
  # rm $coaddir/"exposureMap_initial.fits"
  # rm $coaddir/"exposureMap_tmp.fits"
...
decalsdir=$BDIR/sdss-catalogs
sdssdone=$decalsdir/done_"$filter".txt
if ! [ -d $decalsdir ]; then mkdir $decalsdir; fi
if [ -f $sdssdone ]; then
  echo " "
  echo "Sources are already extracted for $filter band in Sloan image"
  echo " "
else
...
lbtdone=$lbtdir/done_"$filter"_ccd"$h".txt
if ! [ -d $lbtdir ]; then mkdir $lbtdir; fi
if [ -f $lbtdone ]; then
  echo " "
  echo "Sources are already extracted for $filter band in lbt frame for extension $h"
  echo " "
else
  ...
  for a in $(seq 1 $n_exp); do
    ...
    astmkprof apertures_lbt.txt --background=$i --backhdu=1 \
        --clearcanvas --replace --type=int16 --mforflatpix \
        --mode=wcs --output=$lbtdir/entirecamera_"$a"_apertures.fits
        
    astmkcatalog $lbtdir/entirecamera_"$a"_apertures.fits -h1 --zeropoint=0 \
            --valuesfile=$i --valueshdu=1 \
            --ids --ra --dec --magnitude --sum \
            --output=$lbtdir/$base_.fits
    asttable $lbtdir/$base_.fits -h1 --noblank=MAGNITUDE -o$lbtdir/$base
  done
  echo done > $lbtdone
fi
...
matchdir2done=$matchdir2/done_"$filter"_ccd"$h".txt
if ! [ -d $matchdir2 ]; then mkdir $matchdir2; fi
if [ -f $matchdir2done ]; then
  echo " "
  ...
  for a in $(seq 1 $n_exp); do
    base=entirecamera_"$a".fits
    i=$lbtdir/$base
    out=$matchdir2/entirecamera_"$a".cat
    astmatch sdss.cat --hdu=1 $i --hdu=1 --ccol1=RA,DEC --ccol2=RA,DEC --aperture=1/3600 --outcols=aRA,aDEC,aMAGNITUDE,aSUM,bMAGNITUDE,bSUM -o$out
...
alphatruedir=$BDIR/alpha-stars-true
alphatruedone=$alphatruedir/done_"$filter"_ccd"$h".txt
if ! [ -d $alphatruedir ]; then mkdir $alphatruedir; fi
if [ -f $alphatruedone ]; then
  echo " "
  ...
  for a in $(seq 1 $n_exp); do
    base=entirecamera_"$a".fits
    f=$matchdir2/entirecamera_"$a".cat
    alphatruet=$alphatruedir/entirecamera_"$a".txt
    asttable $f -h1 --range=MAGNITUDE,20,25 -o$alphatruet
    asttable $alphatruet -h1 -c1,2,3,'arith $4 $6 /' -o$alphatruedir/$base
    mean=$(asttable $alphatruedir/$base -c'ARITH_1'| aststatistics  --sigclip-median)
    std=$(asttable $alphatruedir/$base -c'ARITH_1'| aststatistics --sigclip-std)
    echo "$mean $std" > $alphatruedir/alpha_entirecamera_"$a".txt;
...
muldone=$muldir/done_"$filter"_ccd"$h".txt
if ! [ -d $muldir ]; then mkdir $muldir; fi
if [ -f $muldone ]; then
    echo " "
    ...
    for a in $(seq 1 $n_exp); do
        base=entirecamera_"$a".fits
        f=$entiredir/$base
        alpha_cat=$alphatruedir/alpha_entirecamera_"$a".txt
        alpha=$(awk 'NR=='1'{print $1}' $alpha_cat)
        astarithmetic $f -h1 $alpha x float32 -o $muldir/$base
        # add keywords in this header
        da=$(awk 'NR=='1'{print $1}' $filtereyWordDirectory/"$objectName"_Sloan-"$filter"_"$a"_ccd0.txt)
        filter=$(awk 'NR=='1'{print $2}' $filtereyWordDirectory/"$objectName"_Sloan-"$filter"_"$a"_ccd0.txt)
        t=$(awk 'NR=='1'{print $3}' $filtereyWordDirectory/"$objectName"_Sloan-"$filter"_"$a"_ccd0.txt)
        astfits $muldir/$base -h1 --write=DATE-OBS,"$da" --write=EXPTIME,"$t" --write=FILTER,"$filter"
...
skyfile=$skydir/sky_Sloan-"$filter"_ccd"$h".txt
noiseskydone=$noiseskydir/done_"$filter"_ccd"$h".txt
if ! [ -d $noiseskydir ]; then mkdir $noiseskydir; fi
if [ -f $noiseskydone ]; then
  echo " "
...
python3.9 find_rms_min.py "$filter" 1 $n_exp $h $INDIR $objectName
...
wdir=$BDIR/weight-dir
wdone=$wdir/done_"$filter"_ccd"$h".txt
if ! [ -d $wdir ]; then mkdir $wdir; fi

wonlydir=$BDIR/only-w-dir
wonlydone=$wonlydir/done_"$filter"_ccd"$h".txt
if ! [ -d $wonlydir ]; then mkdir $wonlydir; fi
if [ -f $wdone ]; then
  echo " "
  ...
  for a in $(seq 1 $n_exp); do
    ...
    f=$muldir/$base
    rms_min=$(awk 'NR=='1'{print $1}' $BDIR/rms_min_val-1_ccd"$h".txt)
    rms_f=$(awk 'NR=='1'{print $2}' $noiseskydir/entirecamera_"$a".txt)
    weight=$(astarithmetic $rms_min $rms_f / --quiet)
    echo "$weight" > $wdir/entirecamera_"$a".txt    #  saving into file
    # multiply each image for its weight
    wixi_im=$wdir/$base           # frame x weight
    w_im=$wonlydir/$base           # only weight
    astarithmetic $f -h1 $weight x -o$wixi_im
...
if [ -f $clippingdone ]; then
    ...
    med_im=$clippingdir/median_image.fits
    std_im=$clippingdir/std_image.fits
    astarithmetic $(ls $wdir/*.fits) $(ls $wdir/*.fits | wc -l) 2 0.2 sigclip-mean -g1 -o$med_im
    astarithmetic $(ls $wdir/*.fits) $(ls $wdir/*.fits | wc -l) 2 0.2 sigclip-std -g1 -o$std_im
    # Compute "borders" images
    up_lim=$clippingdir/upperlim.fits
    lo_lim=$clippingdir/lowerlim.fits
    thresh=$clippingdir/threesigmalim.fits
    astarithmetic 3. $std_im x -o$thresh
...
# w*I
mowdir=$BDIR/weight-dir-no-outliers
mowdone=$mowdir/done_"$filter"_ccd"$h".txt
if ! [ -d $mowdir ]; then mkdir $mowdir; fi

# only weight
moonwdir=$BDIR/only-weight-dir-no-outliers
moonwdone=$moonwdir/done_"$filter"_ccd"$h".txt
if ! [ -d $moonwdir ]; then mkdir $moonwdir; fi
...
if [ -f $mowdone ]; then
    ...
    for a in $(seq 1 $n_exp); do
        base=entirecamera_"$a".fits
        tmp_ab=$mowdir/entirecamera_"$a"_maskabove.fits
        wom=$mowdir/$base
        ...
        # save the new mask
        mask=$mowdir/entirecamera_"$a"_mask.fits
...
coaddone=$coaddir/done_"$filter".txt
if ! [ -d $coaddir ]; then mkdir $coaddir; fi
if [ -f $coaddone ]; then
    echo " "
    ...
    astarithmetic $(ls $mowdir/*.fits) $(ls $mowdir/*.fits | wc -l) sum -g1 -o$coaddir/"$filter"_wx.fits
    astarithmetic $(ls $moonwdir/*.fits ) $(ls $moonwdir/*.fits  | wc -l) sum -g1 -o$coaddir/"$filter"_w.fits
    astarithmetic $coaddir/"$filter"_wx.fits -h1 $coaddir/"$filter"_w.fits -h1 / -o$coaddir/"$objectName"_Sloan-"$filter".fits
    echo done > $coaddone
fi
