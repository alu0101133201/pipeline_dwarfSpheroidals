...
def getPanstarrsQuery(tra, tdec, size, filters="grizy", format="fits", imagetypes="stack"):
...
def download_field_image(field_id,ra,dec,downloadDestination,band='r',data_release='DR17'):
    ...
    try:
        ...
        print(f"SDSS query failed for field {field_id} at RA: {ra}, Dec: {dec}. Error: {e}")
        return
    if not images:
        print(f"No images found for field {field_id} at RA: {ra}, Dec: {dec}.")
        return
