...
# This was the first approach used for identifying the bad frames. It ended up being not very useful
# and we moved to use the background Value normalised by the airmass (counts / airmass)
# and use that distribution to identify bad frames
...
def obtainPotentialBadFiles(dataDict, medianValues, stdValues):
    ...
    for currentKey in dataDict:
        ...
        for currentImageValue in currentValues:
            if (np.abs(currentImageValue[1] - median) >= (numerOfStdForRejecting*std)):
...
numerOfStdForRejecting    = sys.argv[4]
