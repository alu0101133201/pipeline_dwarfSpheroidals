...
export swarpcfg # I export this and here I pass it as argument cos I have modified something below, before it was not exported. Have no time to change it, remove the argument from warpImage()
...
calibrateOurData() {
  ...
  brightLimit=16.3
  faintLimit=18.5
  computeCalibrationFactors $alphatruedir $matchdir2 $brightLimit $faintLimit
...
iteration=1
imagesForCalibration=$entiredir
calibrateOurData $iteration $imagesForCalibration $selectedDecalsStarsDir $rangeUsedDecalsDir $frameChosenBrickMap $decalsImagesDir
...
if [ -f $matchdir2done ]; then
  ...
  for a in $(seq 1 $n_exp); do
    ...
    astmatch sdss.cat --hdu=1 $i --hdu=1 --ccol1=RA,DEC --ccol2=RA,DEC --aperture=$toleranceForMatching/3600 --outcols=aRA,aDEC,aMAGNITUDE,aSUM,bMAGNITUDE,bSUM -o$out
